...
class QueueController extends ChangeNotifier {
  ...
  // 🛠️ 3. LA LÓGICA DE NEGOCIO Y BASE DE DATOS
  ...
  Future<void> handleSync(BuildContext context) async {
    ...
    try {
      ...
      if (context.mounted) UIHelpers.showSnackBar(context, "⚠️ Error sincronizando: $e", AppTheme.error);
    } finally {
      isSyncing = false;
      notifyListeners();
    }
  ...
  // 🖱️ 4. LA LÓGICA DE SELECCIÓN (Multi-Select)
  ...
  Future<void> deleteSelected(BuildContext context) async {
    ...
    try {
      ...
      if (context.mounted) UIHelpers.showSnackBar(context, "🗑️ $count foto(s) eliminada(s) correctamente", AppTheme.error);
    } catch (e) {
      if (context.mounted) UIHelpers.showSnackBar(context, "⚠️ Error al eliminar fotos en bloque: $e", AppTheme.error);
    }
  }
  ...
  Future<void> processDeletion(BuildContext context, int photoId) async {
    try {
      ...
      if (context.mounted) UIHelpers.showSnackBar(context, "🗑️ Eliminado correctamente", AppTheme.error);
    } catch (e) {
      if (context.mounted) UIHelpers.showSnackBar(context, "⚠️ Error al eliminar: $e", AppTheme.error);
    }
  }
}